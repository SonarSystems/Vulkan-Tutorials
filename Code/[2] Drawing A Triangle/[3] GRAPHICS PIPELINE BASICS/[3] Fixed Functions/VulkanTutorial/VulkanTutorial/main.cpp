...
private:
	...
	*/
	...
	void CreateGraphicsPipeline( )
	{
		...
		* \brief START OF NEW CODE ADDED
		*
		* Implementing fixed functions (in older graphics APIs there were default states for the majority of the stages in a the graphics pipeline. With Vulkan you have to be explicit about everything)
		*/
		// Specify that there is currently no vertex data (we will circle back to this later)
		VkPipelineVertexInputStateCreateInfo vertexInputInfo{ };
		vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
		vertexInputInfo.vertexBindingDescriptionCount = 0;
		vertexInputInfo.vertexAttributeDescriptionCount = 0;

		// Set what type of geometry is to be drawn and where or not primitive restart is enabled which is useful for triangle strips
		...
		// Viewport describes the region of the framebuffer that the output will be rendered to. This will almost always be (0, 0) to (screen width, screen height)
		VkViewport viewport{ };
		viewport.x = 0.0f;
		viewport.y = 0.0f;
		...
		// How much of the framebuffer is to be drawn, we will be drawing all of it
		VkRect2D scissor{ };
		scissor.offset = { 0, 0 };
		scissor.extent = _swapChainExtent;

		// Now we combine the viewport and scissor rectangle
		...
		// For now it is disabled but we will check back on it later
		...
		//		2: Combine the old and new value using a bitwise operation (to use set logicOpEnable to VK_TRUE)
		...
		// Used for altering the behaviour of shaders with recreating them (will circle back to this later)
